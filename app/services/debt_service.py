...
try:
    # If you have an IMF ratio helper, we'll use it; otherwise we silently skip it.
    from app.providers.imf_provider import imf_debt_to_gdp_annual  # type: ignore
except Exception:  # pragma: no cover
    imf_debt_to_gdp_annual = None
...
def compute_debt_payload(country: str) -> Dict[str, Any]:
    """
    Port of your original /v1/debt route logic, unchanged in behavior.
    Strict compute order:
    ...
    try:
        ...
        # 2) IMF WEO annual ratio
        if best is None and imf_debt_to_gdp_annual is not None:
            try:
        ...
        # If still nothing, return empty ratio (series include Eurostat if we have it)
        if best is None:
            return {
                "country": country,
                "iso_codes": codes,
        ...
        elif path_used == "IMF_ANNUAL_RATIO" and imf_debt_to_gdp_annual is not None:
            try:
