# app/routes/probe.py — TINY + health endpoints (startup-safe)
...
# --- Health + Root (instant, no deps) ----------------------------------------
@router.get("/", summary="Root")
def root() -> Dict[str, Any]:
    return {"ok": True, "service": "country-radar", "hint": "see /healthz, /ping, /docs"}

@router.get("/healthz", summary="Health check")
def healthz() -> Dict[str, Any]:
    # keep it boring + instant
    return {"status": "ok"}
...
# --- Minimal diagnostics (still instant) -------------------------------------
...
@router.get("/__provider_fns", summary="List callables in a provider module")
